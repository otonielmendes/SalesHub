import type { BacktestMetrics, DistributionEntry, ParsedRow, RiskEntry, VelocityEntry } from "@/types/backtest";
...
function riskEntries(
  ...
  for (const row of rows) {
    const key = keyFn(row);
    if (!key) continue;
    const entry = map.get(key) ?? { total: 0, fraud: 0 };
    entry.total++;
...
export function calculateMetrics(rows: ParsedRow[]): BacktestMetrics {
  ...
  const fraudInApprovedToday = hasFraud
    ? approvedToday.filter((r) => r.fraud).length
    : 0;
  const fraudInApprovedKoin = hasFraud
    ? acceptedKoin.filter((r) => r.fraud).length
    : 0;
  ...
  const recoverable =
    hasPaymentStatus && hasKoinDecision && hasFraud
      ...
      : [];
  const recoverableVolume = hasAmount
    ? recoverable.reduce((sum, r) => sum + (r.amount ?? 0), 0)
    : 0;
  ...
  // --- Risk tables ---
  const riskByItem = rows[0].item !== null
    ? riskEntries(rows, (r) => r.item)
    : null;

  const riskByBin = rows[0].bin !== null
    ? riskEntries(rows, (r) => r.bin, 5, 1)
    : null;

  const riskByEmail = rows[0].email !== null
    ? riskEntries(rows, (r) => r.email)
    : null;

  const riskByDocument = rows[0].document !== null
    ? riskEntries(rows, (r) => r.document)
    : null;
  ...
  if (rows[0].document !== null) {
    ...
    highVelocityDocuments = Array.from(docMap.entries())
      .filter(([, v]) => v.total >= 10)
      .map(([doc, v]) => ({ document: doc, total: v.total, fraudCount: v.fraud }))
      .sort((a, b) => b.total - a.total)
      .slice(0, 20);
  }

  // --- Distributions ---
  const cardBrandDistribution = rows[0].cardBrand !== null
    ? distribution(rows, (r) => r.cardBrand)
    : null;

  const deliveryDistribution = rows[0].delivery !== null
    ? distribution(rows, (r) => r.delivery)
    : null;

  const devolucionCount = hasPaymentStatus
    ? rows.filter((r) => isDevolucion(r.paymentStatus ?? "")).length
    : null;
