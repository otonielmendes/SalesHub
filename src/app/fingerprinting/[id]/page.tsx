...
import { DemoDetailClient } from "@/app/fingerprinting/_components/DemoDetailClient";
  ...
  return (
    <div className="mx-auto max-w-container px-6 py-8 lg:px-8">
      ...
      <nav aria-label="Breadcrumb" className="mb-10 flex flex-wrap items-center gap-3 text-sm text-[#475456]">
        <Link href="/fingerprinting/history" className="rounded-md p-1 transition-colors hover:bg-[#EAECEE]" aria-label={tc("homeAria")}>
          <HomeLine className="h-5 w-5" />
        </Link>
        <span className="text-[#D0D5D7]">/</span>
        <Link href="/fingerprinting/history" className="rounded-md px-2 py-1 font-medium transition-colors hover:bg-[#EAECEE]">
